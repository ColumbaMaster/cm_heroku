...
function convertDecimalParaGraus(val, tipo) {
  /*
  var convCoord = Math.abs(val);
  var coordDeg = Math.floor(convCoord);
  var coordMin = (Math.floor((convCoord - coordDeg) * 60));
  var coordSeg = (Math.floor((coordDeg - coordMin) * 60)).toFixed();
  */
  /*
  var convCoord = Math.abs( Math.round(val * 1000000.));
  var coordDeg = Math.floor(convCoord / 1000000);
  var coordMin = Math.floor(((convCoord/1000000) - coordDeg) * 60);
  var coordSeg = ( Math.floor((( ((convCoord/1000000) - coordDeg) * 60) - coordMin) * 100000) *60/100000 ).toFixed();
  */
  ...
  document.getElementById(tipo +'dd').value = coordDeg;
  document.getElementById(tipo +'dm').value = coordMin;
  document.getElementById(tipo +'ds').value = coordSeg;
  document.getElementById(tipo +'direction').value = coordCardinal;
