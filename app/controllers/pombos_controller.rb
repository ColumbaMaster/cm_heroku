class PombosController < ApplicationController
  ...
  def criar(tipo, sexo, label, incluir)
    ...
    pombo.cor_id = 11 if pombo.cor_id.blank?
  ...
  private
    ...
    def pombo_params
      params.require(:pombo).permit(:anilha, :nome, :dtnasc, :sexo, :cor_id, :usuario_id, :pai_id, :mae_id, :vivo)
    end
    
    def parente_params(tipo)
      params.require(tipo).permit(:anilha, :nome, :dtnasc, :sexo, :cor_id, :vivo, :pai_id, :mae_id)
    end
