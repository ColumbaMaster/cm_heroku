class ProvasController < ApplicationController
  ...
  def importados
    ...
    msgs =  ResultadoImportado.import_file(params[:file], params[:id_usuario_filter], id_usu, @prova.id)
    if msgs.size == 0
  ...
  def create_result
    ...
    if params[:opcao_result] == 'P'
      @result.dhrestimada = date_transform('dhrestimada')
    else
      @result.dhroficial = date_transform('dhroficial')
  ...
  def create
    ...
    @prova.usuario = current_user
    @prova.update_attributes(lat: dms_to_degrees(@prova.lat_deg, @prova.lat_min, @prova.lat_seg, @prova.lat_card), 
  ...
  private
    ...
    def date_transform(nome)
      Time.zone.parse(
        params['resultado']["#{nome}(1i)"] + '-' +
        params['resultado']["#{nome}(2i)"] + '-' +
        params['resultado']["#{nome}(3i)"] + ' ' +
        params['resultado']["#{nome}(4i)"] + ':' +
        params['resultado']["#{nome}(5i)"] + ':' +
        params['resultado']["#{nome}(6i)"]
      )
